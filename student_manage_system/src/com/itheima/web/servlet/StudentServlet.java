...
public class StudentServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        }else if("firstPage".equalsIgnoreCase(method)){
            //首页
            firstPage(req,resp);
        }else if("lastPage".equalsIgnoreCase(method)){
            //上一页
            lastPage(req,resp);
        }else if("nextPage".equalsIgnoreCase(method)){
            //下一页
            nextPage(req,resp);
        }else if("tailPage".equalsIgnoreCase(method)){
            //尾页
            tailPage(req,resp);
        }else{
    ...
    private void firstPage(HttpServletRequest req,HttpServletResponse resp)throws ServletException, IOException{
        try{
            ...
            PageInfo<Student> pageinfo = (PageInfo)session.getAttribute("pageinfo");
            pageinfo.setPageNum(1);
            List<Student> students = studentService.findAll(req,resp);
            //2.存入请求域中
    ...
    private void lastPage(HttpServletRequest req,HttpServletResponse resp)throws ServletException, IOException{
        try{
            HttpSession session = req.getSession();
            PageInfo<Student> pageinfo =(PageInfo<Student>) session.getAttribute("pageinfo");

            if(pageinfo.getPrePage()!=0){
                pageinfo.setPageNum(pageinfo.getPrePage());
    ...
    private void nextPage(HttpServletRequest req,HttpServletResponse resp)throws ServletException, IOException{
        try{
            HttpSession session = req.getSession();
            PageInfo<Student> pageinfo =(PageInfo<Student>)session.getAttribute("pageinfo");

            if(pageinfo.getNextPage()!=0){
                pageinfo.setPageNum(pageinfo.getNextPage());
                session.setAttribute("pageinfo",pageinfo);
    ...
    private void tailPage(HttpServletRequest req,HttpServletResponse resp)throws ServletException, IOException{
        try{
            HttpSession session = req.getSession();
            PageInfo<Student> pageinfo =(PageInfo<Student>) session.getAttribute("pageinfo");
